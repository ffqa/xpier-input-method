...
final class XpierPanel: NSPanel {
  private let view: XpierView
  private let back: NSView
  var inputController: XpierInputController?
  ...
  /// FR-2: 候选窗是否跟随光标（style/panel_follow）。false = 固定在当前屏幕底部居中。
  private var followsCaret: Bool = true

  init(position: NSRect) {
    self.position = position
    self.view = XpierView(frame: position)
  ...
  func load(config: XpierConfig, forDarkMode isDark: Bool) {
    // FR-2: 跟随方式可调（style/panel_follow，缺省跟随光标）
    followsCaret = config.getBool("style/panel_follow") ?? true
    if isDark {
      view.darkTheme = XpierTheme()
      view.darkTheme.load(config: config, dark: true)
    } else {
      view.lightTheme = XpierTheme()
      view.lightTheme.load(config: config, dark: isDark)
    }
  }
}

private extension XpierPanel {
  ...
  func show() {
    ...
    if requiresFullScreen {
      ...
      if vertical {
        ...
        if position.midY / screenRect.height >= 0.5 {
          panelRect.origin.y = position.minY - XpierTheme.offsetHeight - panelRect.height + theme.pagingOffset
        } else {
          panelRect.origin.y = position.maxY + XpierTheme.offsetHeight
        }
        panelRect.origin.x = position.minX - panelRect.width - XpierTheme.offsetHeight
        ...
        panelRect.origin = NSPoint(x: position.minX - theme.pagingOffset, y: position.minY - XpierTheme.offsetHeight - panelRect.height)
      }
      ...
      if panelRect.minY < screenRect.minY {
        if vertical { panelRect.origin.y = screenRect.minY } else { panelRect.origin.y = position.maxY + XpierTheme.offsetHeight }
      }
    ...
    // FR-2: 固定屏底模式（style/panel_follow: false）——不跟随光标，停靠当前屏幕底部居中
    if !followsCaret {
      panelRect.origin = NSPoint(
  ...
  func show(status message: String) {
    ...
    statusTimer?.invalidate()
    statusTimer = Timer.scheduledTimer(withTimeInterval: XpierTheme.showStatusDuration, repeats: false) { _ in
      self.hide()
    }
  }
