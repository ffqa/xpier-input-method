...
final class XpierConfig {
  ...
  func open(schemaID: String, baseConfig: XpierConfig?) -> Bool {
    close()
  ...
  func getColor(_ option: String, inSpace colorSpace: XpierTheme.RimeColorSpace) -> NSColor? {
...
private extension XpierConfig {
  func cachedValue<T>(of: T.Type, forKey key: String) -> T? {
    return cache[key] as? T
  }

  func color(from colorStr: String, inSpace colorSpace: XpierTheme.RimeColorSpace) -> NSColor? {
  ...
  func color(alpha: Int, red: Int, green: Int, blue: Int, colorSpace: XpierTheme.RimeColorSpace) -> NSColor {
    switch colorSpace {
    case .displayP3:
