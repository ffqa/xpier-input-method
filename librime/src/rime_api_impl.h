...
// FR-4b (Shurufa): 最近一次上屏词立即置顶
static Bool RimePromoteLastCommit(RimeSessionId session_id) {
  an<Session> session(Service::instance().GetSession(session_id));
  if (!session || !session->engine())
    return False;
  return session->engine()->PromoteLastCommit() ? True : False;
}
...
RIME_API RIME_FLAVORED(RimeApi) * RIME_FLAVORED(rime_get_api)() {
  ...
  if (!s_api.data_size) {
    ...
    s_api.promote_last_commit = &RimePromoteLastCommit;
  }
  return &s_api;
}
