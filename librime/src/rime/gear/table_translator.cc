...
bool TableTranslator::Memorize(const CommitEntry& commit_entry) {
  ...
  // FR-4b: 缓存本次上屏词条（拷贝），供 Ctrl+= 手动置顶
  last_shurufa_committed_.clear();
  for (const DictEntry* e : commit_entry.elements) {
    if (is_constructed(e)) {
      DictEntry blessed(*e);
      UnityTableEncoder::RemovePrefix(&blessed.custom_code);
      last_shurufa_committed_.push_back(blessed);
    } else {
      last_shurufa_committed_.push_back(*e);
    }
  }
  ShurufaPromoteIfNeeded(commit_entry);
  if (encoder_ && encoder_->loaded()) {
...
// FR-4a (Shurufa): 会话内连续同词上屏计数；达到阈值即对词条大幅加重并持久
void TableTranslator::ShurufaPromoteIfNeeded(const CommitEntry& commit_entry) {
  if (!user_dict_ || commit_entry.elements.empty() || commit_entry.text.empty())
    return;
  if (commit_entry.text == last_shurufa_commit_text_) {
    ++shurufa_commit_streak_;
  } else {
    last_shurufa_commit_text_ = commit_entry.text;
    shurufa_commit_streak_ = 1;
    shurufa_boosted_this_streak_ = false;
  }
  if (shurufa_commit_streak_ >= kShurufaPromoteThreshold &&
      !shurufa_boosted_this_streak_) {
    shurufa_boosted_this_streak_ = true;
    for (const DictEntry* e : commit_entry.elements) {
      user_dict_->UpdateEntry(*e, kShurufaPromoteCommits);
    }
    DLOG(INFO) << "shurufa promote (FR-4a): " << commit_entry.text;
  }
}

// FR-4b (Shurufa): Ctrl+= —— 最近一次上屏词条再次大幅加重并持久
bool TableTranslator::PromoteLastCommittedEntry() {
  if (!user_dict_ || last_shurufa_committed_.empty())
    return false;
  for (const auto& e : last_shurufa_committed_) {
    user_dict_->UpdateEntry(e, kShurufaPromoteCommits);
  }
  DLOG(INFO) << "shurufa promote (FR-4b): manual top";
  return true;
}
