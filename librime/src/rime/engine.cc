...
class ConcreteEngine : public Engine {
 public:
  ...
  virtual bool PromoteLastCommit();
...
bool ConcreteEngine::PromoteLastCommit() {
  for (auto& translator : translators_) {
    auto* table = dynamic_cast<TableTranslator*>(translator.get());
    if (table && table->PromoteLastCommittedEntry())
      return true;
  }
  return false;
}
